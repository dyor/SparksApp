...
async function findAvailablePort(startPort = 8081) {
  ...
  throw new Error(`Could not find available port (tried ${startPort}-${maxPort})`);
}
...
async function main() {
  ...
  const startPort = parseInt(args.find(arg => arg.startsWith('--port='))?.split('=')[1] || '8081', 10);
  ...
  console.log(`   In Codespaces, check the 'Ports' tab to see the forwarded port.`);
  console.log('');
