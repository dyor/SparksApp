...
const HandicapService = {
    ...
    calculateDifferential(adjustedGrossScore: number, rating: number, slope: number): number {
        return (adjustedGrossScore - rating) * (113 / slope);
    },

    calculateHandicap(differentials: { diff: number, type: '9' | '18' }[]): number | null {
        if (differentials.length < 2) return null;

        // Group 9-hole rounds to form 18-hole differentials
        const nineHoleDiffs = differentials.filter(d => d.type === '9').map(d => d.diff);
        const eighteenHoleDiffs = differentials.filter(d => d.type === '18').map(d => d.diff);
...
}>(({ hole, score, onSave, colors, styles, expectedElapsedTime, beginTime, strokesReceived }, ref) => {
    ...
    return (
        <View style={styles.tableRow}>
            ...
            <View style={styles.parCol}>
                ...
                {strokesReceived ? (
                    <Text style={{ fontSize: 14, lineHeight: 10, color: colors.textSecondary, textAlign: 'center', marginTop: -2 }}>
...
const ScoringView: React.FC<ScoringViewProps> = ({
    ...
    const validDifferentials = finishedRounds
        ...
        .filter(d => !isNaN(d.diff) && isFinite(d.diff) && d.diff > -20 && d.diff < 60) as { diff: number, type: '9' | '18' }[];

    const currentHandicapIndex = HandicapService.calculateHandicap(validDifferentials);
    ...
    const totalToPar = roundScores.reduce((sum, s) => {
        ...
        const strokesReceived = HandicapService.getStrokesForHole(courseHandicap, hole.stroke_index, course.holes.length);
    ...
    return (
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
            ...
            <View style={styles.subHeader}>
                ...
                <TouchableOpacity
                    style={[styles.completeRoundHeaderButton, !canComplete && { backgroundColor: '#cccccc', opacity: 70 }]}
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <HoleRow
                        ...
                        strokesReceived={HandicapService.getStrokesForHole(courseHandicap, item.stroke_index, course.holes.length)}
                    />
                )}
                ListFooterComponent={
                    <View style={styles.tableFooterRow}>
                        ...
                        <Text style={[styles.footerText, styles.netCol]}>
                            {totals.strokes > 0 ? (totalToPar > 0 ? `+${totalToPar}` : totalToPar) : ''}
                        </Text>
...
export const ScorecardSpark: React.FC<{ showSettings?: boolean, onCloseSettings?: () => void }> = ({ showSettings, onCloseSettings }) => {
    ...
    const markRoundComplete = (roundId: number) => {
        ...
        const differential = HandicapService.calculateDifferential(adjustedGross, course.rating, course.slope); // Raw differential, not scaled
