...
const MusicMakerSpark: React.FC<{
    ...
    // Removed Web Audio API vocal source refs in favor of expo-av
    ...
    const playSong = async () => {
        ...
        try {
            ...
            if ((playWithChords || playWithDrums || playIntro) && Platform.OS === 'web') {
                // Use Web Audio API only for synthesized accompaniment
                const AudioContextClass = window.AudioContext || (window as any).webkitAudioContext;
                ...
                if (playIntro) {
                    ...
                    topChords.forEach((chord, idx) => {
                        ...
                        freqs.forEach((freq, i) => {
                            ...
                            const strum = rhythm.strums[i % rhythm.strums.length];
                            const barDuration = beatDuration * 4;
                            const strumTime = strum.time * barDuration;
                    ...
                    for (let t = 0; t < totalDuration; t += (beatDuration * 4)) {
                        pattern.forEach((note: any) => {
                            ...
                            if (note.type === 'kick') {
                                ...
                                osc.type = 'triangle';
                                osc.frequency.setValueAtTime(100, noteStart);
            ...
            if (introDelay > 0) {
                setTimeout(async () => {
                    try { await sound.playAsync(); } catch (e) { console.log('Play failed', e); }
                }, introDelay * 1000);
            } else {
                            ...
                            <FlatList
                                ...
                                renderItem={({ item }: { item: MusicAnalysisResult }) => (
                                    <TouchableOpacity onPress={() => setSelectedSong(item)} style={styles.songItem}>
                                        ...
                                        <Text style={{ color: colors.textSecondary, fontSize: 12 }}>{item.key} • {item.bpm} BPM</Text>
                                    </TouchableOpacity>
                                )}
